...
class SectionsPagerAdapter(private val context: Context, fm: FragmentManager) :
    ...
    override fun getItem(position: Int): Fragment {
        return when (position) {
            0 -> LatestPostsFragment(viewModelFactory, requestManager)
            1 -> HotPostsFragment(viewModelFactory, requestManager)
            2 -> TopPostsFragment(viewModelFactory, requestManager)
